...
async function confirmVote(id, option) {
  ...
  async function getResults(id, option) {
    const data = { key: id, option: option };

  try {
      const response = await fetch('http://127.0.0.1:8001/resultsVote', {
          method: 'POST',
          body: JSON.stringify(data),
          headers: { 'Content-Type': 'application/json' }
      });
  ...
  function addItemList(container, data, type) {
        ...
        confirmButton.addEventListener('click', async function() {
          ...
          if (selectedProposition) {
              ...
              if (result.message === "Vote Sumbitted Successfully!") {
                  ...
                  confirmationMessage.innerHTML = 'Vote confirmed for: <b>' + selectedProposition + '</b><br><br> <div class="confirmation-time">Paillier: '+ result.paillierTimer +'s</div>'+ '<div class="confirmation-time"> THC: '+ result.thcTimer +'s</div>';
              } else {
    ...
    if (type === "Closed") {
      propositions.forEach(async function(proposition) {
          var prop_div = document.createElement("div");
          prop_div.classList = "props-item-closed";
          var voteRes = await getResults(idKey,proposition);
          console.log(voteRes);
          //idKey
          prop_div.innerHTML = '<b>' + proposition + '</b><br>Votes: '+voteRes['result'];
          options_div.appendChild(prop_div);
      });
  }
